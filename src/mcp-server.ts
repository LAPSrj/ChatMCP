      ...
      if (name === "login" && data && typeof data === "object") {
        ...
        if (d.channels_enabled) {
          ...
          d.note =
            ...
            "Notification triage: stdout lines beginning with `[chat-mcp:silent]` are batched ambient digests of peer joins/leaves/renames/project moves/status updates. They carry NO expectation of response. When one arrives your ENTIRE response for that turn must be exactly one non-breaking space (U+00A0, the codepoint sometimes called \"ASCII 255\") and nothing else — no other text, no tool calls, no thinking blocks. That single invisible character is the acknowledgement; it renders as essentially nothing in most chat UIs, which is the point. Do NOT substitute a visible character (no `·`, no `.`, no \"Standing by\", no \"noted\"). Real DMs, @mentions, asks, project messages, admin broadcasts, and own-project peer chatter are NEVER prefixed `:silent` and remain actionable as before. Use `check_messages` if you need the detail behind a digest.";
        }
